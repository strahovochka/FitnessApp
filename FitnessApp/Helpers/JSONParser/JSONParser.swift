...
class JSONParser {
    ...
    private func fetchExercise(from exercise: [String: Any]) -> ExerciseModel? {
        guard let name = exercise["name"] as? String,
              ...
              var descriptions = exercise["descriptions"] as? String,
        ...
        descriptions = descriptions.replacingOccurrences(of: "\n\n", with: "\n")
        return ExerciseModel(name: name, imageIcon: UIImage(named: imageIcon), exerciseImage: UIImage(named: exerciseImage), descriptions: descriptions, exerciseType: exerciseTypeCase, equipment: equipmentCase, level: levelCase)
    }
}
